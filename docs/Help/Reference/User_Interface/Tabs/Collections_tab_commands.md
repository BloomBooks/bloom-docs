...
-   **Import Content from Spreadsheet** ![](/ref-docs-assets/images/User_Interface/BloomEnterprise%20button.png): It [imports](../../Tasks/Basic_tasks/Import_Content_from_Spreadsheet.md) contents from an \*.xlsx spreadsheet that was previously exported.
    ...
    -   **Save as Single File (.bloomSource)**: It updates the book and the book folder is saved in a zip file, renamed as `<name>`.bloom (_instead of_ .zip). Some files are excluded in this zip file, such as files ending in .pdf, .bloombookorder, .map, and .bloompack. You can choose where to save the file.
